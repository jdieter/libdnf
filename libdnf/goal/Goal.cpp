...
static std::map<int, std::string> ERROR_DICT = {
   {MULTIPLE_MATCH_OBJECTS, _("Ill-formed Selector, presence of multiple match objects in the filter")},
   {INCORECT_COMPARISON_TYPE, _("Ill-formed Selector used for the operation, incorrect comparison type")}
};
...
sltrToJob(const HySelector sltr, Queue *job, int solver_action)
{
 ...
 finish:
    queue_free(&job_sltr);
    if (ret > 1) {
        throw libdnf::Goal::Exception(ERROR_DICT[ret], DNF_ERROR_BAD_SELECTOR);
    }
}
